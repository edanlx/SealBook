...
## 4.ConstantPool
<!--串池位置https://blog.csdn.net/weixin_41826973/article/details/105229010-->
常量池即字节码文件中除了包含类的版本信息、字段、方法以及接口描述信息(此部分在元空间)外的常量池表。运行时常量池就是字节码文件中运行的表示形式，这里面包含符号表等多种数据。
* 总结如下：
1. 所有的类字段方法的名称都会被纳入进来
2. 无论是在什么位置写,只要用双引号括起来数据都会被纳入进来，如果一开始没写，运行的时候就要通过intern加载进来。如果是动态接收的会转为StringBuilder，即存在堆。(这里字面量的部分会由StringTable对外提供服务，其本质是大小固定1009的HashSet,里面维护了指针，可通过-XX:StringTableSize进行调整)
3. 如果是基本类型，在方法内部会归入到局部常量表中，在类中写了final会归到常量池中,不写final会在初始化中重新指定。(核心是会走系统级的常量池)
4. 当被加载后，符号引用会被替换为真实引用
```
Constant pool:
