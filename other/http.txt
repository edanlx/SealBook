协议:TCP/IP是通信协议(网络协议)的总称。互联网中具有代表性的协议有IP、TCP、HTTP等。而LAN(局域网)中常用的协议IPX/SPX等。TCP/IP家族主要包含IP、ICMP、TCP、UDP、HTTP、TELNET、SNMP、SMTP等。协议主要由ISO制定国际标准OSI。(需要注意IP是网络协议与IP地址略有不同，IP地址是网络层地址，MAC是数据链路层地址,端口由传输层处理标记上层地址也可以叫做程序地址)
...
HTTP首部
状态码、压缩协议、是否缓存、HttpOnly(防止跨站脚本攻击),content-MD5(防止内容篡改)
HTTPS:与SSL组合使用的HTTP被称为HTTPS(HTTP secure),证书即个人身份的确认，理论上来讲伪造证书是很困难的事情(但在历史上也发生过伪造，特别是非权威机构颁发的证书则更容易被伪造)。HTTP+加密+认证+完整性保护=HTTPS。其通信机制为先于SSL对HTTP加密后通保证通信安全再用TCP通信
SSL:目前为止使用最广泛的组合加密，同时采用了共享秘钥加密和公开秘钥加密，在交换秘钥环节使用公开加密，交换报文则使用处理速度较快的共享秘钥加密防篡改。
【SSL图示、第7章确保Web安全的HTTPS】
【图163】

常见认证方式:
用户名密码、动态令牌、数字证书、生物认证、IC卡

题外话:
对称加密(共享秘钥加密)与非对称加密(公钥秘钥加密,公钥随意发布，私钥不可被人知道)的区别:1、对称加密中加密和解密使用的秘钥是同一个；非对称加密中采用两个密钥，一般使用公钥进行加密，私钥进行解密。2、对称加密解密的速度比较快，非对称加密和解密花费的时间长、速度相对较慢。3、对称加密的安全性相对较低，非对称加密的安全性较高。
websocket:在确立连接时使用HTTP，连接建立后则采用独立的数据帧